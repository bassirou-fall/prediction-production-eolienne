...
def preparer_features(vitesse, direction, temperature, pression, humidite):
    """Prépare X avec le bon shape et types pour le modèle."""
    valeurs = [vitesse, direction, temperature, pression, humidite]
    X = np.array([[float(v) for v in valeurs]], dtype=float)
    if np.isnan(X).any():
        raise ValueError("Entrées invalides: NaN détecté.")
    if X.shape != (1, 5):
        raise ValueError(f"Shape attendu (1, 5), obtenu {X.shape}")
    return X
...
with onglet_prediction:
    ...
    if st.button("🔍 Prédire", type="primary"):
        ...
        try:
            X = np.array([[vitesse, direction, temperature, pression, humidite]], dtype=float)
            X = preparer_features(vitesse, direction, temperature, pression, humidite)
            prediction = modele.predict(X)[0]
        except Exception as e:
            st.error(f"⚠️ Erreur de prédiction : {e}")
            st.stop()
...
# Lien cliquable (si st.page_link n'existe pas, fallback markdown)
url_projet = "https://github.com/bassirou-fall/prediction-production-eolienne"
try:
    st.page_link(url_projet, label="🔗 Lien vers le projet GitHub", icon="🌐")
except Exception:
    st.markdown(f"[🔗 Lien vers le projet GitHub]({url_projet})")
